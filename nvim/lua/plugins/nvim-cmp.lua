return {
    ...
    dependencies = {
        ...
        'onsails/lspkind.nvim', -- vs-code pictorgrams
    ...
    config = function()
        ...
        -- returns true if the cursor is in the leftmost column or at a whitespace char
        ...
        local confirm = function(entry)
            local behaviour = cmp.ConfirmBehavior.Replace
            if entry then
                ...
                -- of the `newText` to the inserted ? if not, then `Insert` instead
                if entry.context.cursor_after_line:sub(1, diff_after) ~= newText:sub(-diff_after) then
                    behaviour = cmp.ConfirmBehavior.Insert
                end
            end
            cmp.confirm({ select = true, behaviour = behaviour })
        end
        ...
        cmp.setup({
            ...
            -- setup lspkind for vscode pictorgrams in autocompletion dropdown menu
            formatting = {
